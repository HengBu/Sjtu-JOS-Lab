...
static struct Page *chunk_list;	//chunk_list is from big addr to small, just like page_free_list


/*
//check function, print the first 20 of free list and chunk list
void printfc(){
	cprintf("free_list:");
	if(!page_free_list) cprintf("null\n");
	else{
	...
	cprintf("chunk_list:");
	if(!chunk_list) cprintf("null\n");
	else{
		struct Page *p = chunk_list;
		int n = 20;
		while(p && n){
			cprintf("%x ", page2pa(p));
			p = p->pp_link;
...
page_init(void)
{
	...
	int upperPgnum = PGNUM(boot_alloc(0)-KERNBASE);;
	page_free_list = NULL;
	
	pages[0].pp_ref = 1;  
	
	for(int i=1;i<npages;i++){
		if(i<npages_basemem || n>=upperPgnum)
		{
			pages[i].pp_ref = 0;
...
page_alloc_npages(int alloc_flags, int n)
{
	if(n <= 0) 
		return NULL;
	struct Page *Pret = page_alloc_npages_helper(&chunk_list, alloc_flags, n);
	if(Pret) 
		return Pret;
	else
		return page_alloc_npages_helper(&page_free_list, alloc_flags, n);
}

struct Page *
page_alloc_npages_helper(struct Page **head, int alloc_flags, int n)
{
	int tmpn = n;
	struct Page *plow = *head;
	struct Page *phigh = *head;
	struct Page *ptmp = NULL;
	struct Page **con_head = NULL;

	while(plow && (tmpn > 1)){
		if(plow->pp_link == NULL) 
			return NULL;
		if(page2pa(plow) - page2pa(plow->pp_link) != PGSIZE){
			//not continuous
			phigh = plow;
			plow = plow->pp_link;
			tmpn = n;
	...
	*con_head=plow->pp_link;
	...
	if(alloc_flags & ALLOC_ZERO)
		 memset(page2kva(phigh), '\0', n * PGSIZE);
	return phigh;

}

// Return n continuous pages to chunk list. Do the following things:
//	1. Check whether the n pages in the list are continue, Return -1 on Error
...
{
	//page will be in order in chunk list
	//check whether pages are continuous
	if(!pp) return -1;
	if(n < 1) return -1;
	struct Page *phigh = pp;
	struct Page *plow = pp;
	...
	for(i= 1; i< n; ++i){
		if(!phigh->pp_link) 
			return -1;
		if(page2pa(phigh->pp_link)-page2pa(phigh) != PGSIZE) 
			return -1;
		phigh = phigh->pp_link;
	}

	//link to chunk list
	struct Page *pcon = chunk_list;
	while(pcon && (phigh < pcon->pp_link))
		pcon = pcon->pp_link;

	//reverse
	struct Page **con_head = &chunk_list;
	if(pcon != chunk_list)
		con_head = &(pcon->pp_link);
	else{
		if(phigh<chunk_list)
			con_head = &(chunk_list->pp_link);
	}	
	
	while((*con_head) != phigh){
		plow = pp->pp_link;
		pp->pp_link = *con_head;
		*con_head = pp;
		pp = plow;
...
{
	//page will be in order in free list
	struct Page *ptmp = NULL;
...
page_realloc_npages(struct Page *pp, int old_n, int new_n)
{
	if(old_n == new_n) 
		...
		for(i= 0; i< new_n-1; ++i)
			pcur = pcur->pp_link;
		//let this one's next be NULL
		ptmp = pcur;
		pcur = pcur->pp_link;
		ptmp->pp_link = NULL;
		//free the rest
		page_free_npages(pcur, old_n-new_n);
		return pp;
	}else{
		struct Page *pcur = pp;
		int i;
		...
		for(i= 0; i< old_n-1; ++i)
			pcur = pcur->pp_link;
		//check whether the behind page is in chunk list or free list
		if(page_realloc_npages_helper(&chunk_list, pcur, old_n, new_n))
			return pp;
		if(page_realloc_npages_helper(&page_free_list, pcur, old_n, new_n))
			return pp;
		//free and alloc
...
page_realloc_npages_helper(struct Page **head, struct Page *pp, int old_n, int new_n){
	...
	struct Page *plow = *head;
	struct Page *phigh = *head;
	while(plow && (pp < plow->pp_link)){
		plow = plow->pp_link;
		ele_num++;
	}
	if(ele_num < new_n - old_n) 
		return 0;
	ele_num = ele_num + old_n - new_n - 1;
	while(ele_num > 0){
		phigh = phigh->pp_link;
		ele_num--;
	}
	
	//phigh not equals *head
	if((ele_num == 0) && (page2pa(phigh->pp_link) - page2pa(pp) != (new_n-old_n)*PGSIZE)) 
		return 0;
	
	//phigh equals *head
	if((ele_num == -1) && (page2pa(phigh) - page2pa(pp) != (new_n-old_n)*PGSIZE)) 
		return 0;
	
	if(ele_num == 0){
		phigh->pp_link = plow->pp_link;		
	}else{
		*head = plow->pp_link;		
	}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if(pp == NULL) 
		return;
	page_decref(pp);  //free will be done in page_decref
